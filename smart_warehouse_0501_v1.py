"""
smart_warehouse_v17_optimal.py
==============================
v17 + 최적화: LB 일반화 우선 설계

핵심:
  - 파일 경로 하드코딩 (C:\\Users\\82108\\Downloads\\open)
  - Optuna 50 trials, sample_frac 0.6
  - LGB 4-seed ensemble
  - 롤링 윈도우 확장 (3,5,7,10,14)
  - 누적 통계 피처, EWM 확장, AE latent 48
  - Stage 2 파라미터 강화
"""
...
# Configuration
# ════════════════════════════════════════════════════════════════
DATA_DIR = r"C:\Users\82108\Downloads\open"
...
# ── Ensemble ──
P_GRID = [0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 5.0]
LGB_SEEDS = [SEED, SEED + 7, SEED + 42, SEED + 100]
...
SCENARIO_AGG_COLS = [
    ...
    "trip_per_robot", "battery_efficiency_product",
]
...
TS_COLS = [
    ...
    "risk_index", "bottle_neck", "order_pressure",
    "battery_risk", "trip_per_robot",
]

EWM_CORE_COLS = [
    "order_inflow_15m", "congestion_score", "pack_utilization",
    "risk_index", "order_per_station", "low_battery_ratio",
]
ROLL_WINDOWS = (3, 5, 7, 10, 14)
...
def add_basic_features(df):
    ...
    if "congestion_score" in df.columns:
        ...
        # 누적 평균 혼잡도
        df["cum_congestion_mean"] = (
            df.groupby("scenario_id")["congestion_score"]
    ...
    # 추가 피처
    if "battery_mean" in df.columns and "robot_efficiency" in df.columns:
        df["battery_efficiency_product"] = df["battery_mean"] * df["robot_efficiency"]
    ...
    # 시나리오 내 분위수
    for col in ["congestion_score", "order_inflow_15m", "pack_utilization"]:
...
def add_timeseries_features(df):
    ...
    for col in TS_COLS:
        ...
        if col in EWM_CORE_COLS:
            ...
            df[f"{col}_ewm_fast"] = (
                s1.groupby(scen).ewm(alpha=0.6, adjust=False)
                .mean().reset_index(level=0, drop=True)
            )
...
def add_interaction_features(df):
    pairs = [
        ...
        ("risk_index", "order_per_station", "risk_x_ops"),
    ]
    ...
    if "timeslot" in df.columns:
        for col in ["congestion_score", "pack_utilization", "order_per_station", "low_battery_ratio", "risk_index"]:
            if col in df.columns:
    ...
    scen_pairs = [
        ...
        ("risk_index_scen_mean", "order_per_station_scen_mean", "risk_ops_interaction"),
        ("bottle_neck_scen_mean", "pack_utilization_scen_mean", "bn_pack_interaction"),
    ]
...
def add_extra_features(df):
    ...
    df["rows_remaining_frac"] = (gsz - gpos) / (gsz + 1e-6)
    ...
    if "timeslot" in df.columns:
        ...
        df["ts_squared"] = df["timeslot"].astype(np.float64) ** 2
...
def apply_target_encoding(train_df, test_df, kf, groups, global_mean):
    ...
    for a, b in te_pairs:
        name = f"{a}_X_{b}"
        _te(name, train_df[a].astype(str) + "_" + train_df[b].astype(str),
...
# ════════════════════════════════════════════════════════════════
class LGBProgress:
    def __init__(self, fold, n_folds, total, every=500):
        self.fold, self.n_folds, self.total, self.every = fold, n_folds, total, every
        self.start, self.best_mae, self.best_it = time.time(), float("inf"), 0

    def __call__(self, env):
        it = env.iteration + 1
        val_mae = env.evaluation_result_list[0][2]
        if val_mae < self.best_mae:
            self.best_mae, self.best_it = val_mae, it
        if it % self.every == 0 or it == self.total:
            pct = it / self.total * 100
            print(
                f"\r  Fold {self.fold}/{self.n_folds} {pct:5.1f}%"
                f"  iter {it:>5}  val_MAE {val_mae:.4f}"
                f"  best {self.best_mae:.4f}@{self.best_it}  {elapsed(self.start)}",
                end="", flush=True,
            )
...
def main():
    ...
    feature_cols_s1_base = [
        c for c in train.columns if c not in ID_COLS + [TARGET]
    ]
    ...
    if USE_OPTUNA_HPO:
        ...
        sample_scens = rng.choice(sample_scens, size=int(len(sample_scens) * OPTUNA_SAMPLE_FRAC), replace=False)
        ...
        def lgb_objective(trial):
            params = dict(
                ...
                num_leaves=trial.suggest_int("num_leaves", 63, 4095),
            ...
            X_tr = train_sample.iloc[tr_idx][feature_cols_s1_base]
            y_tr = y_sample.iloc[tr_idx]
            X_val = train_sample.iloc[val_idx][feature_cols_s1_base]
            ...
            return mae(y_val_raw, from_train_pred(m.predict(X_val)))
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(train, y_strat, groups=groups), 1):
        ...
        print(f"    Fold {fold} MAE: LGB={mae(y_raw[va_idx], oof_s1_lgb[va_idx]):.4f}"
    ...
    if USE_STAGE2:
        ...
        lgb_params_s2 = dict(
            objective="regression_l1", n_estimators=15000, learning_rate=0.015,
            max_depth=-1, num_leaves=1023, min_child_samples=60,
            subsample=0.8, subsample_freq=1, colsample_bytree=0.7,
            reg_alpha=0.1, reg_lambda=3.0, random_state=SEED, verbose=-1,
        )
        xgb_params_s2 = dict(
            objective="reg:absoluteerror", n_estimators=10000, learning_rate=0.015,
            max_depth=7, subsample=0.8, colsample_bytree=0.7, colsample_bynode=0.5,
            reg_alpha=0.1, reg_lambda=2.0, random_state=SEED, tree_method="hist",
            eval_metric="mae", early_stopping_rounds=150, verbosity=0,
        )
        cat_params_s2 = dict(
            iterations=10000, learning_rate=0.015, depth=7, l2_leaf_reg=3.0,
            bootstrap_type="MVS", subsample=0.8, colsample_bylevel=0.7,
            loss_function="MAE", eval_metric="MAE",
            random_seed=SEED, task_type="CPU", early_stopping_rounds=150,
        )
        ...
        for fold, (tr_idx, va_idx) in enumerate(kf.split(train, y_strat, groups=groups), 1):
            ...
            print(f"    LGB={mae(y_raw[va_idx], oof_s2_lgb[va_idx]):.4f}"
    ...
    save_path = os.path.join(DATA_DIR, "submission_v17_optimal.csv")
