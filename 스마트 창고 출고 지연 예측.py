# -*- coding: utf-8 -*-
# ============================================================
# SmartWarehouse 최종 통합 코드
# - 기반: smart_warehouse_v17_optimal (가장 낮은 OOF/LB)
# - 보완: SmartWareHouse_0430의 dtype 안정화, OOF calibration, timeslot alpha
# - 참고: 실행I의 hard fold / clipping 분석을 진단 파일로 반영
#
# 개발 환경
#   OS      : Windows 11 Home
#   Python  : 3.14.0
...
# ── Stage 2 ──
USE_STAGE2 = True
BLEND_ALPHA_STEP = 0.01  # coarse — 과적합 방지
CLIP_Q = 0.995           # 기본 고정 clip

# ── 보완 옵션: SmartWareHouse_0430 / 실행I에서 유효했던 안전장치 ──
ENABLE_TIMESLOT_ALPHA_SEARCH = True
TIMESLOT_ALPHA_STEP = 0.02
TIMESLOT_ALPHA_SMOOTH_TO_GLOBAL = 0.20
ENABLE_GLOBAL_CALIBRATION_SEARCH = True

# ── Ensemble ──
P_GRID = [1.0, 1.5, 2.0, 3.0, 4.0]
LGB_SEEDS = [SEED, SEED + 100]  # multi-seed (2 seeds)
...
def search_global_calibration(oof_pred, y_true):
    ...
    best_params = {"scale": 1.0, "bias": 0.0, "clip_q": None, "clip_upper": None}
    rows = []
    ...
    calib_df = pd.DataFrame(rows).sort_values("mae")
    return best_params, best_score, calib_df


def encode_remaining_objects(train_df, test_df, exclude=("ID",)):
    """layout_type 등 남은 문자열 컬럼을 train/test 결합 기준으로 숫자 인코딩한다."""
    ...
    for c in common:
        ...
        if (not pd.api.types.is_numeric_dtype(train_df[c])) or (not pd.api.types.is_numeric_dtype(test_df[c])):
            combined = pd.concat([train_df[c], test_df[c]], axis=0).astype(str).fillna("missing")
...
# ════════════════════════════════════════════════════════════════
# Feature Engineering (v16b + v5 결합)
# ════════════════════════════════════════════════════════════════
SCENARIO_AGG_COLS = [
...
def preprocess_all(df, layout_df):
    ...
    if USE_DOMAIN_CLIP:
        df = clip_domain(df)
    if "layout_type" in df.columns and not pd.api.types.is_numeric_dtype(df["layout_type"]):
        df["layout_type"] = pd.factorize(df["layout_type"].astype(str).fillna("missing"))[0].astype(np.int32)
    # 모델 입력 오류 방지: ID/TARGET 외 남은 object는 숫자화
    for _c in df.columns:
        if _c not in ID_COLS + [TARGET] and not pd.api.types.is_numeric_dtype(df[_c]):
            df[_c] = pd.factorize(df[_c].astype(str).fillna("missing"))[0].astype(np.int32)
    return df
...
def main():
    ...
    train, test = encode_remaining_objects(train, test, exclude=("ID",))
    ...
    # SmartWareHouse_0430 보완점: timeslot별 S1/S2 alpha 탐색
    if USE_STAGE2 and ENABLE_TIMESLOT_ALPHA_SEARCH and "timeslot" in train.columns:
        ts_arr = train["timeslot"].values
        ts_oof_blend = np.zeros(len(train), dtype=np.float64)
        ...
        if ts_mae < final_mae:
            ...
            oof_final = ts_oof_final
            final_mae = ts_mae
            FINAL_METHOD = "timeslot_alpha"

    # SmartWareHouse_0430 보완점: OOF 기준 global calibration, 개선될 때만 적용
    ...
    if USE_STAGE2:
        ...
        if FINAL_METHOD == "timeslot_alpha" and len(ts_alpha_map) > 0 and "timeslot" in test.columns:
            ...
            pred_blend = test_alpha * pred_s1 + (1.0 - test_alpha) * pred_s2
        else:
    ...
    if USE_CALIBRATION:
        pred_final, _ = apply_scale_bias_clip(
            pred_final,
            calib_params["scale"],
            calib_params["bias"],
            calib_params["clip_q"],
