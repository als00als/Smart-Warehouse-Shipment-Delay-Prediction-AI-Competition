...
def main():
    ...
    if not src_candidates:
        raise FileNotFoundError("Could not find *0415_preproc_speedup_exp.ipynb in workspace root")
    ...
    preproc_cell = """# Preprocessing: speedup baseline + scenario-relative feature experiment
# - (x - scen_median) / (scen_iqr + eps)  -> {col}_scen_robust_z
# - x / (scen_mean + eps)                 -> {col}_scen_ratio_mean
# - rank within scenario (percentile)     -> {col}_scen_rank_pct
...
# Scenario-relative experiment (subset of numeric base metrics; edit as needed)
RELFEATURE_EPS = 1e-6
RELFEATURE_COLS = [
    'congestion_score', 'order_inflow_15m', 'pack_utilization', 'avg_trip_distance',
    'robot_efficiency', 'order_per_station', 'low_battery_ratio', 'outbound_truck_wait_min',
    'max_zone_density', 'sku_concentration', 'robot_total', 'robot_active', 'robot_idle',
...
def add_scenario_relative_features(df):
    \"\"\"Per-scenario robust z-score, mean ratio, and rank percentile for selected columns.\"\"\"
    if not ENABLE_SCENARIO_RELATIVE_FEATURES:
        return df
    g = df.groupby('scenario_id', sort=False)
    for col in RELFEATURE_COLS:
        if col not in df.columns:
            continue
        if not pd.api.types.is_numeric_dtype(df[col]):
            continue
        x = df[col]
        scen_med = g[col].transform('median')
        scen_q25 = g[col].transform(lambda s: s.quantile(0.25))
        scen_q75 = g[col].transform(lambda s: s.quantile(0.75))
        scen_iqr = scen_q75 - scen_q25
        df[f'{col}_scen_robust_z'] = (x - scen_med) / (scen_iqr + RELFEATURE_EPS)

        scen_mean = g[col].transform('mean')
        df[f'{col}_scen_ratio_mean'] = x / (scen_mean + RELFEATURE_EPS)

        df[f'{col}_scen_rank_pct'] = g[col].rank(pct=True)
    return df
...
section('Preprocess (speedup + scenario-relative features experiment)')
t0 = time.time()
...
print(f'▶ ENABLE_SCENARIO_RELATIVE_FEATURES={ENABLE_SCENARIO_RELATIVE_FEATURES}, RELFEATURE_COLS={len(RELFEATURE_COLS)}')
"""

    new_nb["cells"][12]["source"] = _to_source_lines(preproc_cell)

    # Inject flags into config cell (index 2) if missing
    cell2 = "".join(new_nb["cells"][2].get("source", []))
    if "ENABLE_SCENARIO_RELATIVE_FEATURES" not in cell2:
        insert = """

# Scenario-relative feature experiment (robust-z, ratio-to-mean, rank pct)
ENABLE_SCENARIO_RELATIVE_FEATURES = True
"""
        cell2 = cell2 + insert
        new_nb["cells"][2]["source"] = _to_source_lines(cell2)

    src_path = Path(src)
    dst_name = src_path.name.replace("_preproc_speedup_exp.ipynb", "_preproc_relfeat_exp.ipynb")
