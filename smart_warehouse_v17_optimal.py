...
==============================
v16b + v5 notebook 최적 조합 — LB 일반화 우선 설계

핵심 변경:
  - v16b: Optuna HPO, trip_per_robot 등 고유 피처, 추가 scenario agg
  - v5 notebook: 2-stage pipeline, AutoEncoder, Target Encoding,
                 Sample Weight, Missing Indicator, Domain Clip,
                 Multi-seed LGB, Power-weighted ensemble
  - 제거: timeslot alpha, meta ridge, fine-grained grid search
  - 보수적 blending (step=0.01), 고정 clip, 5-fold StratifiedGroupKFold
"""
...
CLIP_Q = 0.995           # 고정 clip — sweep 하지 않음
...
def _resolve_data_dir() -> str:
    here = Path(__file__).resolve().parent if "__file__" in dir() else Path.cwd().resolve()
    for p in [here, *here.parents]:
        d = p / "data"
        if d.is_dir() and (d / "train.csv").is_file():
            return str(d)
    raise FileNotFoundError("data/train.csv not found — data 폴더를 확인하세요")
...
def main():
    ...
    save_path = os.path.join(project_root, "submission_v17_optimal.csv")
