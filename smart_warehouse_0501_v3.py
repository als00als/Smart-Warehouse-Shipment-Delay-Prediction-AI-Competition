"""
smart_warehouse_0501_v3.py
==========================
v1 대비 일반화 강화 — AutoEncoder 유지 버전

변경 핵심 (AE 제거 없이 OOF-LB 갭 해소):
  - AE 유지, 단 regularize: dropout 0.2, latent 24, hidden 128
  - scen_rank 제거 (시나리오 전체 미래 정보 의존 → 일반화 저해)
  - 시나리오 집계: mean+std만 (max/min 제거)
  - N_FOLDS 5→7
  - LGB seeds 4→2
  - 전 모델 정규화 강화 (min_child_samples, reg_lambda, path_smooth)
  - 샘플 가중치 보수적으로
  - timeslot interaction 축소
  - TE smoothing 강화
"""
...
# ── AutoEncoder — 정규화 강화 ──
AE_LATENT_DIM = 24        # 48→24: 압축률 높여 일반적 표현 학습
AE_HIDDEN_DIM = 128       # 256→128
AE_EPOCHS_MAX = 40        # 60→40
AE_ES_PATIENCE = 5
AE_BATCH_SIZE = 4096
AE_LR = 1e-3
AE_WEIGHT_DECAY = 1e-4    # 3e-5→1e-4: 가중치 감쇠 강화
AE_DROPOUT = 0.2          # 0.0→0.2: 드롭아웃으로 일반화
AE_DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# ── Target Encoding ──
TE_SMOOTHING = 30         # 20→30: 더 보수적
...
class TabularAutoEncoder(nn.Module):
    def __init__(self, n_in, hidden, latent, dropout=0.0):
        ...
        self.dec = nn.Sequential(
            nn.Linear(latent, hidden // 2), nn.ReLU(inplace=True),
            nn.Dropout(p=dropout),
...
def add_basic_features(df):
    ...
    # scen_rank 제거: 시나리오 전체 정보 의존 → 일반화 저해
    return df
...
def main():
    ...
    save_path = os.path.join(DATA_DIR, "submission_0501_v3.csv")
