...
# - 기반: smart_warehouse_v17_optimal
# - 보완: dtype 안정화, OOF calibration, timeslot alpha
# ============================================================
...
def apply_target_encoding(train_df, test_df, kf, groups, global_mean):
    ...
    def _te(col_name, tr_key, te_key):
        ...
        smooth_full = (st_full["count"] * st_full["mean"] + TE_SMOOTHING * global_mean) / (
            st_full["count"] + TE_SMOOTHING
        )
...
def main():
    ...
    if not sample_path.is_file():
        raise FileNotFoundError("./data/sample_submission.csv가 없습니다. 제출 양식 파일을 포함해야 합니다.")
    ...
    pred_s1 = sum(
        w_s1[m] * {"lgb": p_lgb, "xgb": p_xgb, "cat": p_cat}[m]
    ...
    if USE_STAGE2:
        ...
        pred_s2 = sum(
            w_s2[m] * {"lgb": p2_lgb, "xgb": p2_xgb, "cat": p2_cat}[m]
    ...
    # sample_submission.csv 순서에 맞춰 저장한다.
    pred_df = pd.DataFrame({"ID": test["ID"].values, TARGET: pred_final})

    if not pred_df["ID"].is_unique:
        raise ValueError("예측 결과의 ID가 중복되어 있습니다.")

    sub = sample_submission.copy()
    sub[TARGET] = sub["ID"].map(pred_df.set_index("ID")[TARGET])

    if sub[TARGET].isna().any():
        missing = sub.loc[sub[TARGET].isna(), "ID"].head().tolist()
        raise ValueError(f"sample_submission 매핑 실패: 예측값이 없는 ID가 있습니다. 예: {missing}")

    save_path = project_root / "submission_v17_hybrid_best_combo_2nd_safe.csv"
    sub.to_csv(save_path, index=False)

    # OOF diagnostics
    oof_path = project_root / "oof_v17_hybrid_best_combo_2nd_safe.csv"
    pd.DataFrame(
        {
            "ID": train["ID"].values,
            "y_true": y_raw,
