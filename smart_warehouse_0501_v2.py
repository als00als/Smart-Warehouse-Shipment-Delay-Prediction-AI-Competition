"""
smart_warehouse_0501_v2.py
==========================
v1 대비 일반화 강화 (OOF-LB 갭 1.26 해소 목표)

변경 핵심:
  - AutoEncoder 제거 (시나리오별 과적합 주요 원인)
  - scen_rank 제거 (시나리오 내 미래 정보 의존)
  - 시나리오 집계: mean+std만 유지 (max/min 제거)
  - N_FOLDS 5→7 (CV 안정성)
  - LGB 시드 4→2개
  - 전 모델 정규화 강화
  - 샘플 가중치 보수적으로
  - 과도한 interaction 피처 일부 제거
"""
...
# ── Stage 2 ──
USE_STAGE2 = True
BLEND_ALPHA_STEP = 0.02   # 0.01→0.02: blending 과적합 방지
CLIP_Q = 0.995

# ── Ensemble ──
P_GRID = [1.0, 1.5, 2.0, 3.0, 4.0]
LGB_SEEDS = [SEED, SEED + 42]   # 4→2 seeds: 앙상블 복잡도 감소

# ── Feature flags ──
USE_AUTOENCODER = False          # 제거: 시나리오 과적합 주원인
...
# ── Sample Weight ──
SW_HIGH_Q = 0.90
SW_HIGH_MULT = 1.10             # 1.25→1.10: 극단값 과중 방지
SW_TS_EDGE_MULT = 1.05          # 1.10→1.05

# ── Target Encoding ──
TE_SMOOTHING = 30               # 20→30: 더 보수적인 smoothing
...
# Feature Engineering
# ════════════════════════════════════════════════════════════════
# 시나리오 집계: mean+std만 (max/min 제거 → 일반화 향상)
SCENARIO_AGG_COLS = [
...
EWM_CORE_COLS = [
    ...
    "risk_index", "order_per_station",
]
ROLL_WINDOWS = (3, 5, 10)   # 7,14 제거: 피처 수 감소
...
def add_basic_features(df):
    ...
    # scen_rank 제거: 시나리오 전체 미래 정보 의존 → 일반화 저해
    return df
...
def add_interaction_features(df):
    ...
    for col in ["congestion_score", "order_per_station", "pack_utilization", "risk_index"]:
        sm = f"{col}_scen_mean"
...
def main():
    ...
    print(f"  피처 수: {len(feature_cols_s1)}")
    ...
    if USE_OPTUNA_HPO:
        ...
        def lgb_objective(trial):
            params = dict(
                ...
                min_child_samples=trial.suggest_int("min_child_samples", 50, 200),  # 높게: 일반화
                subsample=trial.suggest_float("subsample", 0.6, 0.9),
                subsample_freq=1,
                colsample_bytree=trial.suggest_float("colsample_bytree", 0.4, 0.8),
                reg_alpha=trial.suggest_float("reg_alpha", 0.1, 3.0),
                reg_lambda=trial.suggest_float("reg_lambda", 1.0, 20.0),  # 높게: 정규화
                min_split_gain=trial.suggest_float("min_split_gain", 0.0, 1.0),
                path_smooth=trial.suggest_float("path_smooth", 0.0, 1.0),  # 신규: 스무딩
                random_state=SEED, verbose=-1,
            )
            tr_idx, val_idx = sample_folds[0]
            X_tr = train_sample.iloc[tr_idx][feature_cols_s1]
            y_tr = y_sample.iloc[tr_idx]
            X_val = train_sample.iloc[val_idx][feature_cols_s1]
        ...
        best_hp = dict(
            learning_rate=0.01, max_depth=7, num_leaves=511,
            min_child_samples=100, subsample=0.75,
            colsample_bytree=0.5, reg_alpha=0.5, reg_lambda=8.0,
            min_split_gain=0.0, path_smooth=0.0,
        )
    ...
    xgb_params = dict(
        ...
        eval_metric="mae", early_stopping_rounds=400, verbosity=0,
    )
    ...
    cat_params = dict(
        ...
        random_seed=SEED, task_type="CPU", early_stopping_rounds=400,
    )
    ...
    if USE_STAGE2:
        ...
        print(f"  S2 피처: {len(feature_cols_s2)}")

        lgb_params_s2 = dict(
            objective="regression_l1", n_estimators=12000, learning_rate=0.015,
            max_depth=best_hp.get("max_depth", 7),
            num_leaves=min(best_hp.get("num_leaves", 511), 511),
            min_child_samples=best_hp.get("min_child_samples", 100),
        ...
        xgb_params_s2 = dict(
            objective="reg:absoluteerror", n_estimators=8000, learning_rate=0.015,
            max_depth=min(best_hp.get("max_depth", 7), 8),
            subsample=0.8, colsample_bytree=0.6, colsample_bynode=0.5,
            reg_alpha=max(best_hp.get("reg_alpha", 0.5), 0.3),
            reg_lambda=max(best_hp.get("reg_lambda", 8.0), 5.0),
            random_state=SEED, tree_method="hist",
            eval_metric="mae", early_stopping_rounds=100, verbosity=0,
        )
        cat_params_s2 = dict(
            iterations=8000, learning_rate=0.015,
            depth=min(best_hp.get("max_depth", 7), 8),
            l2_leaf_reg=max(best_hp.get("reg_lambda", 8.0), 5.0),
            min_data_in_leaf=best_hp.get("min_child_samples", 100),
            bootstrap_type="MVS", subsample=0.8, colsample_bylevel=0.6,
            loss_function="MAE", eval_metric="MAE",
            random_seed=SEED, task_type="CPU", early_stopping_rounds=100,
        )
    ...
    print(f"  blend alpha={best_alpha:.2f}  clip_hi={clip_hi:.2f}")
    ...
    save_path = os.path.join(DATA_DIR, "submission_0501_v2.csv")
